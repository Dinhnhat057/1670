    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $.each(response.data, function (index, value) {
                str += "<tr>";
                str += "<td>" + value.tinTuyenDungId + "</td>";
                str += `<td><a target="_blank" href="/tin-tuyen-dung/${value.tinTuyenDungId}">${value.jobName}</td>`;
                str += "<td>" + value.candidateName + "</td>";
                str += "<td>" + value.recruitmentDate + "</td>";
                str += `<td><a href='/nha-tuyen-dung/UngTuyen/XemCV?id=${value.id}' class='btn btn-success'>Xem hồ sơ</a>`;
                str += `<br /><a href='/nha-tuyen-dung/UngTuyen/CapNhat?id=${value.id}' class='btn btn-warning mt-1'>Cập nhật trạng thái</a></td>`;
                str += "</tr>";
